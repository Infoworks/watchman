...
def main():
    ...
    configure_airflow = raw_input(
        'Configure Airflow? (This will modify Airflow configurations) Answer: yes or no. [' + configure_airflow_default + ']: ')
    ...
    if configure_airflow == 'yes':
        ...
        try:
            ...
            for k in AIRFLOW_CORE_CFG_OVERWRITE:
                print 'Setting [core][%s] to %s (previously: %s)' % (
                k, AIRFLOW_CORE_CFG_OVERWRITE[k], parser.get('core', k))
